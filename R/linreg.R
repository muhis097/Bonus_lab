...
linreg <-setRefClass( Class = "linreg",
                      ...
                      methods=list(
                        ...
                        plot=function(){
                          ...
                          p1<-ggplot()+
                            ggplot2::geom_line(data=data,aes(x=unique(The_fitted_values),y=Ploty_vector),color="red")+
                            ggplot2::geom_point(data=data,aes(x=The_fitted_values,y=The_residuals),shape=1)+
                            ylab("Residuals")+xlab("Fitted values")+
                            labs(title = paste("Residuals vs Fitted values"))+
                            theme(plot.title = element_text(hjust = 0.5))

                          p2<-ggplot()+
                            ggplot2::geom_line(data=data,aes(x=unique(The_fitted_values),y=sqrt(abs(scale(Ploty_vector)))),color="red")+
                            ggplot2::geom_point(data=data,aes(x=The_fitted_values,y=sqrt(abs(scale(The_residuals)))),shape=1)+
